...
class HomeController extends BaseModel {
  ...
  BagelDB _bagelDB = BagelDB(_apiToken);
  ...
  final GlobalKey<FormFieldState> specifyTextFieldKey = GlobalKey<FormFieldState>();
  int tag = 1;
  bool edit = false;
  ...
  void init({Map pizzaMap}) async {
    Pizza pizza;
    if (pizzaMap != null && pizzaMap["pizza"] != null) {
    ...
    if (pizza == null) {
      ...
      currentPizza = pizza;
      pizzaToppingsImages = pizza.toppings.map((Topping topping) => topping.pizzaImage).toList();
    }
    notifyListeners();
  }

  void changePizzaSize(int pizzaSizeIndex) {
    this.tag = pizzaSizeIndex;
    currentPizza.size = returnPizzaSizeString(pizzaSizeIndex);
    notifyListeners();
  }
  ...
  void loadToppings() async {
    // bagelDB get
    BagelResponse res = await _bagelDB.collection('toppings').everything().get();
    var data = res.data;
    List<dynamic> _toppings = data.map((topObj) => Topping.fromMap(topObj)).toList();
  ...
  Future<Null> loadSizes() async {
    ...
    var data = res.data;
    List<dynamic> _sizes = data.map((sizeObj) => PizzaSize.fromMap(sizeObj)).toList();
    _sizes.forEach((size) {
      sizesObjLst.add(size);
    });
    sizesList = sizesObjLst.map((PizzaSize size) => size.name).toList();
    sizesStringsToDisplay =
        sizesObjLst.map((PizzaSize size) => '${size.name} | ${size.price.toStringAsFixed(0)}\$').toList();
    return notifyListeners();
  }

  void addToppingToPizza(Topping topping) {
    pizzaToppingsImages.add(topping.pizzaImage);
  ...
  void removeTopping(Topping topping) {
    pizzaToppingsImages.remove(topping.pizzaImage);
  ...
  void removePizza(Pizza pizzaToRemove) {
    this.pizzasList.remove(pizzaToRemove);
    notifyListeners();
  }

  void submitForm(Map<String, dynamic> form) async {
    ...
    for (Pizza pizza in pizzasList) {
      try {
        Map<String, dynamic> map = {
          ...
          'toppings': pizza.toppings.map((Topping topping) => {'itemRefID': topping.id}).toList(),
          'price': calculatePizzaPrice(pizza)
        };
        // bagelDB post
        BagelResponse response = await this._bagelDB.collection('pizzas').post(item: map);
    ...
    // bagelDB post
    BagelResponse res = await this._bagelDB.collection('carts').post(item: cartMap);
  ...
  calculatePizzaPrice(Pizza pizza) {
    double sizePrice = sizesObjLst.firstWhere((PizzaSize sizeObj) => sizeObj.name == pizza.size).price;
    double toppingsTotalPrice = 0;
    for (Topping topping in pizza.toppings) {
      toppingsTotalPrice = toppingsTotalPrice + topping.price;
    }
    return toppingsTotalPrice + sizePrice;
  }
